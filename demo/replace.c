...
int   main(int argc, char *argv[])
{
    ...
    conditstr(pstSavm, stUser, user_addr, "earth");              // 查询条件赋值
    conditstr(pstSavm, stUser, user_phone, "1869112XXXY");

    updatenum(pstSavm, stUser, acct_id, 2018053010);             // 查询条件赋值
    updatestr(pstSavm, stUser, user_nm, "DeffPuzzL");            // 查询条件赋值
    updatestr(pstSavm, stUser, user_type, "1");                  // 查询条件赋值
    updatestr(pstSavm, stUser, user_no, "20180229");             // 查询条件赋值
    if(RC_SUCC != lReplace(pstSavm, (void *)&stUser))
    {
        fprintf(stderr, "Update error: (%d) (%s), ep(%d)\n", pstSavm->m_lErrno, 
